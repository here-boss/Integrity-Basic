...
while true; do
  if [ ! -e "${MCTRL}/disable" ] && [ ! -e "${MCTRL}/remove" ]; then
    if is_magisk && [ ! -f /sdcard/stop ]; then
      ...
      # Show if Shamiko just got activated
      if [ "$shamiko_prev" != "on" ] && [ -f "$SHAMIKO_WHITELIST" ]; then
        BOSS "Shamiko Whitelist Mode Activated"
        shamiko_prev="on"
      fi

      # Show if NoHello just got activated
      if [ "$nohello_prev" != "on" ] && [ -f "$NOHELLO_WHITELIST" ]; then
        BOSS "NoHello Whitelist Mode Activated."
        nohello_prev="on"
      fi
    ...
    if [ -f "$SHAMIKO_WHITELIST" ]; then
      ...
      BOSS "Shamiko Blacklist Mode Activated."
      shamiko_prev="off"
    fi

    if [ -f "$NOHELLO_WHITELIST" ]; then
      rm -f "$NOHELLO_WHITELIST"
      BOSS "NoHello Blacklist Mode Activated."
      nohello_prev="off"
    fi
  fi
  sleep 4
...
# Module install path
export MODPATH="/data/adb/modules/integrity_basic"
