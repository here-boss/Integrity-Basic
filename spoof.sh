...
D="$MODDIR/integrity_basic"
L="$D/integrity_basic.log"
...
update_json() {
    ...
    if [ "$spoof_value" -eq 0 ]; then
        ...
        BOSS "spoofVendingSdk removed from $json_file"
    else
        ...
        if [ "$json_file" == "$PIF_FORK_PATH" ]; then
            ...
            BOSS "spoofVendingSdk added to custom.pif.json"
        else
            ...
            BOSS "spoofVendingSdk added to pif.json"
        fi
    fi
}
