...
L="/data/adb/Integrity-Basic-Logs/detection.log"
...
ROM_TYPE="- Not Found"
unzip -l /system/etc/security/otacerts.zip | grep -q "testkey" && ROM_TYPE="testkey (Unverified)" 
...
ip route | grep -q "tun0" && VPN_STATUS="Active"
ps -A | grep -q "proxydroid" && PROXY_STATUS="Active"
log "   ├─ VPN: $VPN_STATUS"
...
dumpsys activity service ClipboardService | grep -q "hasPrimaryClip=true" && log "   └─ Detected!" || log "   └─ Not Found"
log "$Q"
log " "

# FAKE GPS DETECTION
log "- GPS Spoofing"
dumpsys location | grep -q "mock" && log "   └─ Detected!" || log "   └─ Not Found"
log "$Q"
log " "
...
[ -n "$UNTRUSTED_CA" ] && log "   └─ Detected!" || log "   └─ Not Found"
log "$Q"
log " "
